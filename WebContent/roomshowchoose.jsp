...
<table class="table">
    ...
    <tr>
        ...
        <% 
        int typenum=0;
        if(type.equals("单人房"))typenum=1;
        else if(type.equals("双人房"))typenum=2;
        else typenum=3;                
        %>
        <td><%
        if(status.equals("空闲")&&manager.rank!=1){
        %>
        <a  href=nowlistinsert.jsp?id=<%=id %>&price=<%=price %>>入住登记</a>
        <%;}%> 
        </td>
        <td><%
        if(manager.rank!=1){
        %>
        <a  href=reservationinsert.jsp?id=<%=id %>&price=<%=price %>&typenum=<%=typenum %>&deposit=<%=deposit %>>预订登记</a>
        <%;}%> 
        </td>
    </tr>
